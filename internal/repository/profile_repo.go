...
func scanProfileRow(row *sql.Row) (*models.Profile, error) {
	...
	profile.ProfileHierarchy = &models.ProfileHierarchy{}
...
func (repo *ProfileRepository) insertApplicationData(profileId string, apps []models.ApplicationData) error {
...
func (repo *ProfileRepository) FetchApplicationData(profileId string) ([]models.ApplicationData, error) {
	...
	var apps []models.ApplicationData
	for rows.Next() {
		var (
			appId     string
			appBlob   []byte
			appParsed models.ApplicationData
		)
		...
		apps = append(apps, models.ApplicationData{
			AppId:           appId,
...
func (repo *ProfileRepository) fetchApplicationDataWithAppId(profileId string, appId string) (models.ApplicationData, error) {
	...
	var app models.ApplicationData
	if err != nil {
...
func (repo *ProfileRepository) UpsertAppDatum(profileId string, appId string, updates map[string]interface{}) error {
	...
	for key, incomingVal := range updates {
		...
		if actualKey == "devices" {
			// Convert to []models.Devices
			devicesJSON, _ := json.Marshal(incomingVal)
			var newDevices []models.Devices
...
func (repo *ProfileRepository) InsertMergedMasterProfileAppData(profileId string, newAppCtx models.ApplicationData) error {
	_, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	updated := false
	var resultAppData []models.ApplicationData
...
func (repo *ProfileRepository) UpdateParent(master models.Profile, targetProfile models.Profile) error {
	_, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	if profileToUpdate.ProfileHierarchy == nil {
		profileToUpdate.ProfileHierarchy = &models.ProfileHierarchy{}
	}
...
func (repo *ProfileRepository) FindProfileByUserName(userName string) (*models.Profile, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (repo *ProfileRepository) AddChildProfiles(parentProfile models.Profile, children []models.ChildProfile) error {
	_, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (repo *ProfileRepository) FetchChildProfiles(parentProfileId string) ([]models.ChildProfile, error) {
