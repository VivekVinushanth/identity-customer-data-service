...
func EnrichProfile(event models.Event) error {
	...
	for _, rule := range rules {
		...
		if !EvaluateConditions(event, rule.Trigger.Conditions) {
			log.Println("Conditions not met for rule %s", rule.RuleId)
			continue
		}
...
func defaultInsertAppData(event models.Event, profile *models.Profile, profileRepo *repositories.ProfileRepository) error {
	...
	deviceList := []models.Devices{devices}
		...
		for _, existingMasterProfile := range existingMasterProfiles {
			...
			if doesProfileMatch(existingMasterProfile, newProfile, rule) {
				...
				if len(existingMasterProfile.ProfileHierarchy.ChildProfiles) == 0 {
					...
					children := []models.ChildProfile{childProfile1, childProfile2}
					...
					children := []models.ChildProfile{newChild}
...
func mergeAppData(existingAppData, incomingAppData []models.ApplicationData, rules []models.ProfileEnrichmentRule) []models.ApplicationData {
